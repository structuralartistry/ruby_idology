...
module RubyIdology
  module IDVerification
    ...
    class Service
      include RubyIdology::Base # for error classes
      ...
      def locate(subject)
        # locate is an IDology ExpectID RubyIdology call - only checks to see if a person is available in the system
        # if available, further RubyIdology calls can be made to verify the person's identity via ExpectID IQ questions

        # new SearchRequest Object
        search_request = RubyIdology::IDVerification::SearchRequest.new

        # assemble the data in a hash for the POST
        search_request.set_data(subject)

        # make the call
        response = RubyIdology::IDVerification::SearchResponse.new( ssl_post(search_request.url, search_request.data) )
      ...
      def get_questions(subject)
        # get_questions is an IDology ExpectID IQ RubyIdology call - which given a valid idNumber from an ExpectID RubyIdology call
        ...
        question_request = RubyIdology::IDVerification::VerificationQuestionsRequest.new

        # assemble the data in a hash for the POST
        question_request.set_data(subject)

        # make the call
        response = RubyIdology::IDVerification::VerificationQuestionsResponse.new( ssl_post(question_request.url, question_request.data) )
      ...
      def submit_answers(subject)
        # submit questions / answers to the IDology ExpectID IQ RubyIdology

        verification_request = RubyIdology::IDVerification::VerificationRequest.new

        # assemble the data for POST
        verification_request.set_data(subject)

        # make the call
        response = RubyIdology::IDVerification::VerificationResponse.new( ssl_post(verification_request.url, verification_request.data) )
      ...
      def get_challenge_questions(subject)
        # get_challenge_questions is an IDology ExpectID Challenge RubyIdology call - given a valid idNumber from an ExpectID IQ question
        # and response process, will return questions to further verify the subject

        question_request = RubyIdology::IDVerification::ChallengeQuestionsRequest.new

        # assemble the data
        question_request.set_data(subject)

        # make the call
        response = RubyIdology::IDVerification::ChallengeQuestionsResponse.new( ssl_post(question_request.url, question_request.data) )
      ...
      def submit_challenge_answers(subject)
        # submit question type / answers to the IDology ExpectID Challenge RubyIdology

        challenge_verification_request = RubyIdology::IDVerification::ChallengeVerificationRequest.new

        # assemble the data
        challenge_verification_request.set_data(subject)

        # make the call
        response = RubyIdology::IDVerification::ChallengeVerificationResponse.new( ssl_post(challenge_verification_request.url, challenge_verification_request.data) )
    ...
    class Subject
      ...
      def initialize(data = nil)
        self.api_service = RubyIdology::IDVerification::Service.new
